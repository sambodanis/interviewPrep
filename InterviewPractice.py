...
def mergeBitsBetweenIndices(N, M, i, j):
...
def nParens(n, l, r, st):
	...
	if l == 0 and r == 0:
		...
		if l > 0:
			st[n] = "("
			nParens(n + 1, l - 1, r, st)
		if r > l:
			st[n] = ")"
			nParens(n + 1, l, r - 1, st)
...
def main():
	...
	# printBinary(N)
	# printBinary(M)
	# printBinary(mergeBitsBetweenIndices(N, M, 0, 6)) # 5.1
	# print bitCountConvert(N, M) # 5.5 
	...
	# arr = [2, -8, 3, -2, 4, -10]
	# print largestContSum(arr) # 19.7 O(n^2)
	# print largestContSumLinear(arr) # 19.7 O(n)
